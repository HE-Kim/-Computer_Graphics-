...
void Display()
{
	...
	glm::mat4 scale_sun = glm::scale(glm::vec3(1.f + 0.5f*abs(sin(g_elaped_time_s)), 1.f + 0.5f*abs(sin(g_elaped_time_s)), 1.f + 0.5f*abs(sin(g_elaped_time_s))));
	glUniformMatrix4fv(m_model_loc, 1, GL_FALSE, glm::value_ptr(scale_sun));
	
	DrawCube();


	//// 큐브 2. //earth
	glm::mat4 trans_x = glm::translate(glm::vec3(2.5f, 0.f, 0.f));
	glm::mat4 scale_half = glm::scale(glm::vec3(0.4f, 0.4f, 0.4f));
	glm::mat4 rot_earth = glm::rotate(glm::radians((360/2)*g_elaped_time_s), glm::vec3(0.f, 1.f, 0.f));
	glm::mat4 rot_around_sun = glm::rotate(glm::radians((360/20)*g_elaped_time_s), glm::vec3(0.f, 1.f, 0.f));

	//glm::mat4 earth = rot_around_sun* trans_x * rot_earth * scale_half;
	glm::mat4 earth = rot_around_sun* trans_x  * rot_earth * scale_half;
   ...
   glm::mat4 scale_m = glm::scale(glm::vec3(0.5f, 0.5f, 0.5f));
   glm::mat4 rot_m = glm::rotate(glm::radians((360 / 2) * g_elaped_time_s), glm::vec3(0.f, 1.f, 0.f));
   glm::mat4 trans_m = glm::translate(glm::vec3(1.f, 0.f, 0.f));
   //glm::mat4 rot_a_e = glm::rotate(glm::radians((360 / 2) * g_elaped_time_s), glm::vec3(0.f, -1.f, 0.f));

   glm::mat4 T3 = earth*trans_m* scale_m;
	...
	glm::mat4 scale_a = glm::scale(glm::vec3(0.3f, 0.3f, 0.3f));
	glm::mat4 rot_a = glm::rotate(glm::radians((360/10) * g_elaped_time_s), glm::vec3(0.f, 1.f, 0.f));
	//360/72=5
	
	for (int i = 0; i <= 360; i += 5)
	{
		glm::mat4 trans_a = glm::translate(glm::vec3(4.5f*cos(glm::radians(float(i))), 0.f, 4.5f*sin(glm::radians(float(i)))));
		glm::mat4 rot_around_sun_a = glm::rotate(glm::radians(float(i)), glm::vec3(0.f, -1.f, 0.f));
		//옆으로 돌려줘여함!  5 도씩 'ㅁ'
		//glm::mat4 T4 = rot_a* trans_a * scale_a;

		glm::mat4 T4 = rot_a*  trans_a *rot_around_sun_a* scale_a;
